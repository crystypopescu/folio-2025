...
import { ArrowHelper, Matrix3, Object3D, Quaternion, Vector3 } from 'three/webgpu'
...
export class RapierRaycastVehicle {
    ...
    updateFriction(delta) {
        ...
        for (let i = 0; i < this.wheels.length; i++) {
            ...
            if (wheel.state.sideImpulse !== 0) {
                const chassisBody = this.chassisRigidBody
                const groundBody = wheel.state.groundRigidBody // TODO
                ...
                rollInfluenceAdjustedWorldPos['xyz'[this.indexUpAxis]] *= wheel.options.rollInfluence
...
function vectorToWorldFrame(object, localVector, target = new Vector3()) {
    const quaternion = vectorToWorldFrame_quaternion.copy(
        object instanceof Object3D ? object.quaternion : (object.rotation()),
    )
